...
struct CustomMaterial {
    ...
    #[uniform(2)]
    noise_color_intensity: f32,
    alpha_mode: AlphaMode,
}

impl Material for CustomMaterial {
    fn fragment_shader() -> ShaderRef {
        "shaders/07_randomness.wgsl".into()
    }
...
enum DevUIControlMessage {
    NoiseColorIntensityChanged(f32),
}
        ...
        parent.spawn(labeled_slider(
            ...
            |event: On<ValueChange<f32>>,
             mut message_writer: MessageWriter<DevUIControlMessage>| {
                message_writer.write(DevUIControlMessage::NoiseColorIntensityChanged(event.value));
            },
        ));
    });
}
...
fn on_dev_control_change(
    ...
    for message in messages.read() {
        match message {
            DevUIControlMessage::NoiseColorIntensityChanged(value) => {
                if let Some(mat) = materials.get_mut(&material.0) {
                    mat.noise_color_intensity = *value;
                }
            }
        }
    }
